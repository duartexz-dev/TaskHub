...
function ocultar() {
    document.getElementById("cardOcultar").style.display = "none";
}

function Abrir() {
    document.getElementById("cardOcultar").style.display = "grid";
}
...
function confirmtask() {
    ...
    let dia = document.getElementById("taskDay").value.trim();
    ...
    taskDiv.className = "task-item glass-card fade-in";
    taskDiv.innerHTML = `
        <span class="task-name">${name}</span>
        <span class="task-name">Dia: ${dia}</span>
        <span class="task-name">Horário: ${hora}</span>
        <div class="task-actions">
            <input type="checkbox" class="task-check" id="check-${taskCounter}">
            <label for="check-${taskCounter}" class="task-check-label">Concluída</label>
        </div>
    `;
    ...
    // Checkbox
    let checkbox = taskDiv.querySelector(".task-check");
    let spanElements = taskDiv.querySelectorAll(".task-name");
    checkbox.addEventListener("change", () => {
        spanElements.forEach(span => {
            span.style.color = checkbox.checked ? "green" : "";
    ...
    function updateProgress() {
        let tasks = list.querySelectorAll(".task-item");
        ...
        let percent = Math.round((doneCount / tasks.length) * 100);
        ...
        function updateProgress() {
            ...
            if (progressBar2) {
                ...
                progressBar2.textContent = percent + "%";
            }
